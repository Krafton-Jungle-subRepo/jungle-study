...
- 필요 개념보다 더 복잡하게 인식하고 있는 경우(개념이 정확하게 잡히지 않은 경우)
...
- 개념간의 연결이 되지 않는 경우 ex) 2차원 matrix와 graph
...
- **추상적 표현** → 사용자 Interface 제공 ex) sort, heapq
...
- ADT와 자료구조의 이름은 일치할수도, 아닐수도 있음 → 많이 헷갈리는 이유 1

- 추상적 표현과 구현된 형상이 완전히 나뉘지 않음  → 많이 헷갈리는 이유 2

  - ex) ADT stack과 C++의 stack / ADT priority queue와 구현된 형상인 heap, python의 heapq
...
1. 규칙에 따라 Node를 Data Structure로 옮긴다
  ...
  - 규칙: 방문한 node에서 갈 수 있는 모든 node를 Dat현
